...
def check_second(name, first, method)
  ...
  results = first.flat_map do |human|
    CLIENT.send(method, human.login).reject do |x|
      first_ids.include? x.id
    end
    print '.'
    sleep 1
  end
...
first_humans = (CLIENT.following + CLIENT.followers).uniq { |x| x.id }
...
second_degree.each { |x| puts "#{x.login} #{x.html_url}" }
puts
